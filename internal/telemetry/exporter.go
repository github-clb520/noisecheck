...
func initOTLPProviders(ctx context.Context, res *resource.Resource, cfg Config) {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "[NC] WARNING: failed to create OTLP trace exporter: %v\n", err)
		return
	}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "[NC] WARNING: failed to create OTLP metric exporter: %v\n", err)
		return
	}
...
func initConsoleProviders(res *resource.Resource) {
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "[NC] WARNING: failed to create console trace exporter: %v\n", err)
		return
	}
	...
	if err != nil {
		fmt.Fprintf(os.Stderr, "[NC] WARNING: failed to create console metric exporter: %v\n", err)
		return
	}
