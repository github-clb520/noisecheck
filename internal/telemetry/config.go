// Package telemetry provides OpenTelemetry-based observability for NoiseCheck CLI.
...
type Config struct {
	Enabled      bool   // Master switch; false when NC_ENABLE_TELEMETRY is unset
...
func resolveEnv(cfg *Config) {
	if os.Getenv("NC_ENABLE_TELEMETRY") == "1" {
		cfg.Enabled = true
	}
	...
	if os.Getenv("NC_CONTENT_LOGGING") == "1" {
		cfg.ContentLog = true
	}
}

// telemetrySection matches the telemetry key in ~/.noisecheck/config.json.
...
// HomeConfigPath returns the default path to ~/.noisecheck/config.json.
func HomeConfigPath() string {
	home, err := os.UserHomeDir()
	if err != nil {
		return ""
	}
	return filepath.Join(home, ".noisecheck", "config.json")
}
