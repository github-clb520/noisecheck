...
func sessionJSONLPath(t *testing.T, repoDir, sessionID string) string {
	...
	return filepath.Join(home, ".noisecheck", "sessions", encodeRepoPath(repoDir), sessionID+".jsonl")
}
...
func TestSessionFilePermissions(t *testing.T) {
	...
	sessionDir := filepath.Join(tmpHome, ".noisecheck", "sessions", encodeRepoPath(repoDir))
